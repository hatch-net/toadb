...
PQuery transformStmt(PNode parserTree)
{
    ...
    switch(parserTree->type)
    {
        ...
        case T_UpdateStmt:
            ...
            break;
        case T_DeleteStmt:
            query = transformDeleteStmt((PDeleteStmt)parserTree);
            break;
        case T_CreateStmt:
        case T_DropStmt:
...
PList RangTblRewriteFromClause(PQueryState queryState, PList fromList)
{
    ...
    for(tmpCell = fromList->head; tmpCell != NULL; tmpCell = tmpCell->next)
    {
        ...
        {
            // TODO: hat_error(); 
            hat_error("range table %s is failure.", rangVar->relname);
            return NULL;
        }
...
static int GetQueryId()
{
    return atomic_fetch_add(&g_queryId, 1);
}
...
PNode TargetNodeProcess(PNode node, PQueryState queryState)
{
    ...
    if(NULL == rte)
    {
        hat_error("column %s is not found, maybe check first.", columnRefNode->field);
...
static PNode SearchColumDef(PNode colRef, PList rangTbl, PColumnDefInfo *colDef)
{
    ...
    for(tmpCell = rangTbl->head; tmpCell != NULL; tmpCell = tmpCell->next)
    {
        ...
        if(findCnt > 1)
        {
            hat_log("column %s ambiguous.", columnRefNode->field);
            firstCol = NULL;
            break;
        }
    }
...
PRangTblEntry GetRangTblNode(char *tableName)
{
    ...
    {
        // TODO: hat_error(); 
        hat_error("table %s is not found.", tableName);
        return NULL;
    }
...
PList ProcessAttrList(PList attrList, PQueryState queryState)
{
    ...
    for(tmpCell = attrList->head; tmpCell != NULL; tmpCell = tmpCell->next)
    {
        ...
        {
            /* TODO: ERROR */
            hat_log("attribute Name %s is not found", attrNameNode->attrName);
            break;
        }
...
PQuery transformDeleteStmt(PDeleteStmt deleteStmt)
{
    ...
    queryState->rtable = NULL;
    queryState->rindex = FIRST_RANGTABLE_INDEX;
    queryState->parserTree = (PNode)deleteStmt;

    /* range table list, which generated by target table of delete stmt, is optional. */
    if(NULL != deleteStmt->fromList)
        rangTblList = RangTblRewriteFromClause(queryState, deleteStmt->fromList);
    
    /* relation is mainly operated. */
    rangVar = (PRangeVar)deleteStmt->relation;
    rtableNode = GetRangTblNode(rangVar->relname);
    if(NULL == rtableNode)
    {
        return NULL;
    ...
    /* 这里主要保留表的别名 */
    rtableNode->rangVar = (PRangeVar)deleteStmt->relation; 
    if(rangVar->alias != NULL)
    {
        /* memory free here up to memory context. */
        rangVar->relname = ((PAlias)(rangVar->alias))->aliasname;
    ...
    /* 
     * target list is all columns of mainly delete table .
     */
    query->targetList = NULL;
    queryState->parentTargetList = TargetRewriteAllColumns(query->targetList, queryState);
    query->targetList = queryState->parentTargetList;    

    /* other rang tables add to list. */
    queryState->rtable = MergeList(queryState->rtable, rangTblList);
    query->rtable = queryState->rtable;
    ...
    queryState->qualTargetList = NULL;
    query->joinTree = NULL;
    if(NULL != deleteStmt->whereList)
    {
        queryState->joinTree = QueryJoinTransform(deleteStmt->whereList, queryState);
