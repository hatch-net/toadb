...
PFileHandle CreateFile(char *filename, int mode)
{
	...
	hat_debug("Debug: opentable file path:%s ", filepath);

	// 检查文件是否存在
	if (FileExist(filepath) > 0)
	{
		hat_log("table file %s already exist. err[%d]", filepath, errno);
		return NULL;
	}
	...
	if (err != 0)
	{
		hat_log("create file %s error, maybe space not enough.errno[%d]", filepath, errno);
		return NULL;
	}
...
PFileHandle OpenFile(char *filename, int mode)
{
	...
	if (err != 0)
	{
		hat_log("open file %s error, errno[%d]", filepath, errno);
		return pfh;
	}
...
int DeleteTableFile(char *filename)
{
	...
	if (ret != 0)
	{
		hat_log("unlink file %s ,errno %d ", filepath, errno);
		return -1;
	}
...
char* FileReadPage(PFileHandle fd, int offset, int size)
{
	...
	if (offset >= filelen)
	{
		hat_error("read offset %d oversize file len %d", offset, filelen);
		return NULL;
	}
...
int FileWritePage(PFileHandle fd, int offset, int size, char *pageBuf)
{
	...
	if (offset >= filelen)
	{
		hat_error("write offset %d oversize file len %d", offset, filelen);
		return -1;
	}
...
int FileReadPage(PFileHandle fd, char *page, INT64 offset, int size)
{
	...
	if(realLen < 0)
	{
		if(errno == ENXIO)
		{
			hat_error("required offset %lld is overfile size.", offset);
		}

		hat_error("required offset %lld is other err[%d].", offset, errno);
		return -1;
	}

	/* at the end of file, return 0. */
