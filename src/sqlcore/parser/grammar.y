...
relation_expr: tablename
                {
                    ...
                    node->alias = NULL;
                    $$ = (PNode)node;
                    hat_log("from relation_expr %s  ", $1);
                }
        ;
            ...
            | AS aliasname 
                {
                    ...
                    node->aliasname = $2;
                    $$ = (PNode)node;
                    hat_log("alias_clause AS aliasname %s  ", $2);
                }
            | aliasname
                {
                    PAlias node = (PAlias)CreateNode(sizeof(Alias),T_Alias);
                    node->aliasname = $1;
                    $$ = (PNode)node;
                    hat_log("alias_clause aliasname %s  ", $1);
                }
        ;
name_list:
            {
...
attr_name_list: attr_name
                    {
                        if(NULL != $1)
                        {
                            ...
                            $$ = NULL;
                        hat_log("insert stmt attr_name:%s", $1);
                    }
                | attr_name_list ',' attr_name
                    {
                        if($3 != NULL)
                        {
                            ...
                            $$ = $1;
                        hat_log("insert stmt multi attr_name:%s", $3);
                    }
        ;
...
columnRef:  attr_name 
                    {
                        ...
                        node->tableName = NULL;
                        $$ = (PNode)node;
                        hat_log("columnRef attr_name :%s ", $1); 
                    }
