...
int TableOpen(PTableList tblInfo, char *tableName, ForkType forkNum)
{
    ...
    if(ret < 0)
    {
        hat_log("[%s][%d]table %s information initialize failure.\n", __FUNCTION__,__LINE__,tableName);
        return -1;
    }
...
static int TableInfoInit(PTableList tblInfo)
{
    ...
    if(ret < 0)
    {
        // hat_log("[%s][%d]read table %s-%d first page failure.\n", __FUNCTION__,__LINE__,tblInfo->tableDef->tableName, MAIN_FORK);
        return -1;
    }
    ...
    if (ret < 0)
    {
        hat_log("[%s][%d]read table %s-%d first page failure.\n", __FUNCTION__,__LINE__,tblInfo->tableDef->tableName, GROUP_FORK);
        return -1;
    }
