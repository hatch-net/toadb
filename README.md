...
地址：[手写数据库内核toadb](https://senllang.blog.csdn.net/category_12338586.html)

介绍数据库整体架构，SQL执行流程，重点对于存储架构，SQL解析，SQL执行进行了重点分析； 根据代码的更新进度，详细介绍各模块流程，开发流程。

## postgresql 使用入门与进阶

地址：[postgresql使用入门基础](https://senllang.blog.csdn.net/category_12346885.html)

从入门基础到进阶的使用，熟悉一种数据库的使用，包括数据库安装部署，SQL语法(创建各类型数据库对象，查询的各种变化，数据类型详细介绍等等), 数据库体系，存储架构原理，以及进阶的权限管理，事务控制，触发器，索引等。

通过本教程，对一款成熟数据库使用，SQL语法，SQL应用，以及数据库的原理有一定了解，这样对于数据库内核的开发，会对它的业务流程不再担心。

## 数据库组件

地址：[C语言实战-手写数据库内核组件](https://senllang.blog.csdn.net/category_12720306.html)

主要针对C语言入门之后的实战练习，将数据库中的各种好玩组件拆解出来，每个都可以单独进行编程，原理分析，流程的设计，代码实现，其中会用到一些编程技巧。

经过这一系列的组件理解与编程实战后，对于开发数据库内核，就不再感到陌生。

# 快速使用 

## 1.[toadb数据库服务管理](./doc/toadb_servicemgr.md)

## 2.[toadsql客户端使用](./doc/toadb_client.md)

## 3.[DDL操作](./doc/toadb_ddl.md)

## 4.[DML操作](./doc/toadb_dml.md)

## 5.[事务操作](./doc/toadb_transaction.md)
